	...
	for _, узел := range узлы {
		for _, символ := range слово {
			if ребёнок, ок := узел.Ребёнок[символ]; ок {
				...
				найтиВсеСлова(ребёнок, string(символ), &результаты, 20)
			}
		}
	}
...
func найтиВсеСлова(узел *Узел, текущееСлово string, результаты *[]string, предел int) {
	if len(*результаты) >= предел {
		return
	}
	if узел.Конец {
	...
	for символ, ребёнок := range узел.Ребёнок {
		найтиВсеСлова(ребёнок, текущееСлово+string(символ), результаты, предел)
		if len(*результаты) >= предел {
			return
		}
	}
...
// НайтиСловаСПрефиксом возвращает все слова в Trie, начинающиеся с указанного префикса
func (т *Трие) НайтиСловаСПрефиксом(префикс string, предел int) []string {
	узел := т.Root
	for _, символ := range префикс {
		if ребёнок, ок := узел.Ребёнок[символ]; ок {
			узел = ребёнок
		} else {
			return []string{}
	...
	результаты := []string{}
	найтиВсеСлова(узел, префикс, &результаты, предел)
	return результаты
}